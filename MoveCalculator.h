...
struct Move {
 Move(Board&& b, size_t ox, size_t oy, size_t nx, size_t ny, char p, bool fb) :
   board(std::move(b)), old_x(ox), old_y(oy), new_x(nx), new_y(ny), promotion(p), figure_beaten(fb) {}
 ...
 const bool figure_beaten{false};
};
